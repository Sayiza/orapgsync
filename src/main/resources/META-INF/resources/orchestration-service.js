 ...
 *
 * Main Functions:
 * - startAll(): Orchestrates the complete 32-step migration workflow
...
async function startAll() {
    ...
    try {
        ...
        updateOrchestrationProgress(1, 'Step 1/32: Testing Oracle connection...');
        ...
        updateOrchestrationProgress(2, 'Step 2/32: Testing PostgreSQL connection...');
        ...
        if (isStepEnabled('schemas')) {
            ...
            updateOrchestrationProgress(3, 'Step 3/32: Extracting Oracle schemas...');
            ...
            updateOrchestrationProgress(5, 'Step 4/32: Creating PostgreSQL schemas...');
        ...
        if (isStepEnabled('synonyms')) {
            ...
            updateOrchestrationProgress(7, 'Step 5/32: Extracting Oracle synonyms...');
        ...
        if (isStepEnabled('objects')) {
            ...
            updateOrchestrationProgress(10, 'Step 6/32: Extracting Oracle object types...');
            ...
            updateOrchestrationProgress(13, 'Step 7/32: Creating PostgreSQL object types...');
        ...
        if (isStepEnabled('sequences')) {
            ...
            updateOrchestrationProgress(15, 'Step 8/32: Extracting Oracle sequences...');
            ...
            updateOrchestrationProgress(18, 'Step 9/32: Creating PostgreSQL sequences...');
        ...
        if (isStepEnabled('tables')) {
            ...
            updateOrchestrationProgress(20, 'Step 10/32: Extracting Oracle table metadata...');
            ...
            updateOrchestrationProgress(22, 'Step 11/32: Creating PostgreSQL tables...');
        ...
        if (isStepEnabled('data')) {
            ...
            updateOrchestrationProgress(24, 'Step 12/32: Extracting Oracle row counts...');
            ...
            updateOrchestrationProgress(26, 'Step 13/32: Transferring data...');
            await transferData();
        ...
        if (isStepEnabled('constraints')) {
            ...
            updateOrchestrationProgress(56, 'Step 14/32: Extracting Oracle constraints...');
            ...
            updateOrchestrationProgress(61, 'Step 15/32: Creating PostgreSQL constraints...');
        ...
        // Step 16-17: Indexes
        // After data transfer (building an index over a populated table is far cheaper than
        // maintaining it during the load) and after constraints, so the indexes PostgreSQL creates
        // for primary keys and unique constraints are already visible and are not duplicated.
        if (isStepEnabled('indexes')) {
            // Step 16: Extract Oracle indexes
            updateOrchestrationProgress(72, 'Step 16/32: Extracting Oracle indexes...');
            await extractOracleIndexes();
            await pollCountBadge('oracle-indexes', { requirePositive: false, allowZero: true });
            updateOrchestrationProgress(73, 'Oracle indexes extracted');

            // Step 17: Create PostgreSQL indexes
            updateOrchestrationProgress(74, 'Step 17/32: Creating PostgreSQL indexes...');
            await createPostgresIndexes();
            await pollCountBadge('postgres-indexes', { requirePositive: false, allowZero: true });
            updateOrchestrationProgress(75, 'Index creation completed');
        } else {
            console.log('Step 16-17: Indexes - SKIPPED (disabled by user)');
            updateOrchestrationProgress(75, 'Steps 16-17: Indexes skipped');
        }

        // Step 18: FK Indexes
        // Deliberately after index migration: this step invents indexes Oracle need not have had,
        // so it must see the migrated ones first to avoid duplicating one over the same columns.
        if (isStepEnabled('fk-indexes')) {
            // Step 18: Fill gaps in FK index coverage
            updateOrchestrationProgress(76, 'Step 18/32: Creating FK indexes...');
            ...
            console.log('Step 18: FK Indexes - SKIPPED (disabled by user)');
            updateOrchestrationProgress(76, 'Step 18: FK Indexes skipped');
        }

        // Step 19-20: View Stubs
        if (isStepEnabled('views')) {
            // Step 19: Extract Oracle view definitions
            updateOrchestrationProgress(77, 'Step 19/32: Extracting Oracle view definitions...');
            ...
            // Step 20: Create PostgreSQL view stubs
            updateOrchestrationProgress(79, 'Step 20/32: Creating PostgreSQL view stubs...');
            ...
            console.log('Step 19-20: View Stubs - SKIPPED (disabled by user)');
            updateOrchestrationProgress(80, 'Steps 19-20: View Stubs skipped');
        }

        // Step 21: Synonym Replacement Views
        // Must run after view stubs (targets must exist) but before view/function/trigger
        // implementations, which may reference synonym names the transformer could not resolve.
        if (isStepEnabled('synonym-replacement')) {
            // Step 21: Create PostgreSQL synonym replacement views
            updateOrchestrationProgress(80, 'Step 21/32: Creating PostgreSQL synonym replacement views...');
            await createPostgresSynonymReplacementViews();
            await pollCountBadge('postgres-synonym-replacement', { requirePositive: false, allowZero: true });
            updateOrchestrationProgress(81, 'Synonym replacement view creation completed');
        } else {
            console.log('Step 21: Synonym Replacement Views - SKIPPED (disabled by user)');
            updateOrchestrationProgress(81, 'Step 21: Synonym Replacement Views skipped');
        }

        // Step 22-23: Function Stubs
        if (isStepEnabled('functions')) {
            // Step 22: Extract Oracle functions/procedures
            updateOrchestrationProgress(81, 'Step 22/32: Extracting Oracle functions and procedures...');
            ...
            // Step 23: Create PostgreSQL function/procedure stubs
            updateOrchestrationProgress(83, 'Step 23/32: Creating PostgreSQL function stubs...');
            ...
            console.log('Step 22-23: Function Stubs - SKIPPED (disabled by user)');
            updateOrchestrationProgress(85, 'Steps 22-23: Function Stubs skipped');
        }

        // Step 24-25: Type Method Stubs
        if (isStepEnabled('type-methods')) {
            // Step 24: Extract Oracle type methods
            updateOrchestrationProgress(87, 'Step 24/32: Extracting Oracle type methods...');
            ...
            // Step 25: Create PostgreSQL type method stubs
            updateOrchestrationProgress(90, 'Step 25/32: Creating PostgreSQL type method stubs...');
            ...
            console.log('Step 24-25: Type Method Stubs - SKIPPED (disabled by user)');
            updateOrchestrationProgress(91, 'Steps 24-25: Type Method Stubs skipped');
        }

        // Step 26: Oracle Compatibility Layer
        if (isStepEnabled('oracle-compat')) {
            // Step 26: Install Oracle compatibility layer
            updateOrchestrationProgress(92, 'Step 26/32: Installing Oracle compatibility layer...');
            ...
            console.log('Step 26: Oracle Compatibility Layer - SKIPPED (disabled by user)');
            updateOrchestrationProgress(93, 'Step 26: Oracle Compatibility Layer skipped');
        }

        // Step 27: View Implementation (SQL Transformation - ACTIVE)
        if (isStepEnabled('view-implementation')) {
            // Step 27: Create PostgreSQL views (view implementation - replaces stubs with actual SQL)
            updateOrchestrationProgress(94, 'Step 27/32: Creating PostgreSQL views...');
            ...
            console.log('Step 27: View Implementation - SKIPPED (disabled by user)');
            updateOrchestrationProgress(95, 'Step 27: View Implementation skipped');
        }

        // Step 28: Function Implementation (PL/SQL Transformation - FROZEN)
        if (isStepEnabled('function-implementation')) {
            // Step 28: Create PostgreSQL standalone functions (function implementation - replaces stubs with actual PL/pgSQL)
            updateOrchestrationProgress(96, 'Step 28/32: Creating PostgreSQL standalone functions...');
            ...
            console.log('Step 28: Function Implementation - SKIPPED (disabled by user)');
            updateOrchestrationProgress(97, 'Step 28: Function Implementation skipped');
        }

        // Step 29: Type Method Implementation (PL/SQL Transformation - FROZEN)
        if (isStepEnabled('type-method-implementation')) {
            // Step 29: Create PostgreSQL type methods (type method implementation - replaces stubs with actual PL/pgSQL)
            updateOrchestrationProgress(97, 'Step 29/32: Creating PostgreSQL type methods...');
            ...
            console.log('Step 29: Type Method Implementation - SKIPPED (disabled by user)');
            updateOrchestrationProgress(98, 'Step 29: Type Method Implementation skipped');
        }

        // Step 30-31: Triggers (PL/SQL Transformation - FROZEN)
        if (isStepEnabled('triggers')) {
            // Step 30: Extract Oracle triggers
            updateOrchestrationProgress(95, 'Step 30/32: Extracting Oracle triggers...');
            await extractOracleTriggers();
            await pollCountBadge('oracle-triggers', { requirePositive: false, allowZero: true });
            updateOrchestrationProgress(96, 'Oracle triggers extracted');

            // Step 31: Create PostgreSQL triggers (trigger implementation)
            updateOrchestrationProgress(97, 'Step 31/32: Creating PostgreSQL triggers...');
            await createPostgresTriggers();
            await pollCountBadge('postgres-triggers', { requirePositive: false, allowZero: true });
            updateOrchestrationProgress(98, 'Trigger implementation completed');
        } else {
            console.log('Step 30-31: Triggers - SKIPPED (disabled by user)');
            updateOrchestrationProgress(98, 'Steps 30-31: Triggers skipped');
        }

        // Step 32: Web Gateway Generation
        if (isStepEnabled('web-gateway')) {
            // Step 32: Generate Quarkus web gateway project
            updateOrchestrationProgress(99, 'Step 32/32: Generating web gateway project...');
            await generateWebGateway();
            await pollCountBadge('postgres-web-gateway', { requirePositive: false, allowZero: true });
            updateOrchestrationProgress(100, 'Web gateway generation completed');
        } else {
            console.log('Step 32: Web Gateway Generation - SKIPPED (disabled by user)');
            updateOrchestrationProgress(100, 'Step 32: Web Gateway skipped');
        }
