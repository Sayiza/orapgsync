 ...
 * You may obtain a copy of the License com
 *
...
create_package
    : CREATE (OR REPLACE)? (EDITIONABLE | NONEDITIONABLE)? PACKAGE (schema_object_name '.')?
       package_name ( invoker_rights_clause | accessible_by_clause )?  (
        IS
        | AS
...
package_obj_body
    ...
    | function_body
    | selection_directive
    | select_directive_split_body
    ;

select_directive_split_body
    : selection_directive_head (procedure_body_tail | function_body_tail)
    ;

selection_directive_head
    : DOLLAR_IF condition DOLLAR_THEN (procedure_signature_wo_semicol | function_signature_wo_semicol)? (
        DOLLAR_ELSIF condition (procedure_signature_wo_semicol | function_signature_wo_semicol)?
    )* (DOLLAR_ELSE (procedure_signature_wo_semicol | function_signature_wo_semicol)? )? DOLLAR_END
    ;
procedure_signature_wo_semicol
    : PROCEDURE identifier ('(' parameter ( ',' parameter)* ')')? (
        accessible_by_clause
        | PARALLEL_ENABLE
    )*
...
nested_table_type_def
    : TABLE OF ( type_spec | '(' type_spec ')' ) (NOT NULL_)?
    ;
...
subprog_decl_in_type
    : modifier_clause* (MEMBER | STATIC) (proc_decl_in_type | func_decl_in_type )
    ;
...
modifier_clause
    : NOT? (INSTANTIABLE | FINAL | OVERRIDING | PERSISTABLE)
    ;
...
subprogram_spec
    : modifier_clause* (MEMBER | STATIC) (type_procedure_spec | type_function_spec)
    ;
...
type_elements_parameter
    : parameter_name (IN | OUT | IN OUT | OUT NOCOPY | IN OUT NOCOPY)?  type_spec DETERMINISTIC? default_value_part?
    ;
...
classification_clause
    ...
    // to handle - 'rule contains a closure with com least one alternative that can match an empty string'
...
xmltype_view_clause
    ...
    //: OF id_expression xml_schema_spec? WITH OBJECT (IDENTIFIER | ID) (
        DEFAULT
...
xmltype_table
   ...
   // : OF id_expression ('(' object_properties ')')? (id_expression xmltype_storage)? xmlschema_spec? xmltype_virtual_columns? (
...
xmltype_column_properties
    ...
    //: id_expression COLUMN? column_name xmltype_storage? xmlschema_spec?
    ;
...
index_org_table_clause
    ...
    | index_org_overflow_clause // rule move_table_clause contains an optional block with com least one alternative that can match an empty string
    ;
...
field_spec
    : column_name type_spec? (NULL_ | (NOT NULL_))? default_value_part?
    ;
...
table_type_def
    : TABLE OF type_spec (NULL_ | (NOT NULL_))? table_indexed_by_part?
    ;
...
precision_part
    : '(' (numeric | id_expression | ASTERISK) (',' (numeric | id_expression | numeric_negative))? (CHAR | BYTE)? ')'
    ;
