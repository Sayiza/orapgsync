...
import me.christianrobert.orapgsync.transformation.semantic.expression.StandardFunction;
...
public class VisitStandardFunction {
    ...
    public static SemanticNode v(PlSqlParser.Standard_functionContext ctx, SemanticTreeBuilder b) {
        if (ctx == null) {
        ...
        if (ctx.string_function() != null) {
            SemanticNode stringFunction = b.visit(ctx.string_function());
            return new StandardFunction(stringFunction);
        }
