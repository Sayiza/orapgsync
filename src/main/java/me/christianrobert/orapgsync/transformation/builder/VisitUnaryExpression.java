...
import me.christianrobert.orapgsync.transformation.semantic.expression.UnaryExpression;

public class VisitUnaryExpression {
  public static SemanticNode v(PlSqlParser.Unary_expressionContext ctx, SemanticTreeBuilder b) {
    ...
    if (ctx.standard_function() != null) {
      SemanticNode standardFunction = VisitStandardFunction.v(ctx.standard_function(), b);
      return new UnaryExpression(standardFunction);
    }
    ...
    if (atomCtx != null) {
      SemanticNode atom = b.visit(atomCtx);
      return new UnaryExpression(atom);
    }
